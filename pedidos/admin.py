...
class PedidoAdmin(admin.ModelAdmin):
    list_display = ['numero_pedido', 'usuario', 'total_pedido', 'forma_pagamento', 'parcelas_formatadas', 'endereco_resumido', 'criado_em', 'listar_itens']
    list_filter = ['forma_pagamento', 'criado_em']
    search_fields = ['usuario__username', 'id']
    inlines = [ItemPedidoInline]
    readonly_fields = ['numero_pedido','usuario', 'forma_pagamento', 'parcelas', 'endereco', 'criado_em', 'atualizado_em']
    ...
    def listar_itens(self, obj):
        return ", ".join([f"{item.quantidade}x {item.produto.titulo}" for item in obj.itens.all()])
